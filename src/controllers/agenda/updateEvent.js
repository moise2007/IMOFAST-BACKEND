...
const updateEvenement = async (req, res) => {
    try {
        ...
        const { type, titre, start, heure, duree, end, description } = req.body
        ...
        await db.collection("evenement").doc(evenementDoc.id).update({
            type, titre, start, heure, duree, end, description,
            updatedAt: new Date()
        })

        const event = {...evenementDoc.data(), type, titre, start, heure, duree, end, description}
